  ...
  if($_POST){ 
    ...
    //codigo
      ...
      $prepare = $pdo->prepare("UPDATE `sala01` SET `nome`=:nome, `data_de_nascimento`=:data_de_nascimento, `serie`=:serie, `turma`=:turma WHERE id=:id");
      ...
      header("Location: sala01.php?mensagem=O aluno ". $nome ." foi atualizado com sucesso!");
      ...
      if($_GET && isset($_GET["id"])){
          try {
            ...
            //$query = "SELECT * FROM";
            $id = $_GET["id"];
            $registros = $pdo->query("SELECT * FROM sala01 WHERE id='$id'");
