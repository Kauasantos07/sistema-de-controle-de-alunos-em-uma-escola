  ...
  if($_POST){ 
    ...
    //codigo
      ...
      $prepare = $pdo->prepare("UPDATE `sala03` SET `nome`=:nome, `data_de_nascimento`=:data_de_nascimento, `serie`=:serie, `turma`=:turma WHERE id=:id");
      ...
      header("Location: sala03.php?mensagem=O aluno ". $nome ." foi atualizado com sucesso!");
      ...
      if($_GET && isset($_GET["id"])){
          try {
            ...
            //$query = "SELECT * FROM";
            $id = $_GET["id"];
            $registros = $pdo->query("SELECT * FROM sala03 WHERE id='$id'");
